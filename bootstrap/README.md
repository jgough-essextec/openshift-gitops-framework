# Bootstrap Directory - MOVED# OpenShift Cluster Bootstrap Instructions

**⚠️ This directory has been reorganized to follow documentation patterns.**This document provides step-by-step instructions for bootstrapping an OpenShift cluster with Argo CD using this GitOps repository.

## New Locations## Prerequisites

### Cluster Bootstrap- OpenShift cluster (OKD 4.12+ or OpenShift 4.12+)

- `oc` CLI tool configured and authenticated to the cluster

The bootstrap documentation has moved to:- `kubectl` CLI tool (optional, but recommended)

- Cluster admin privileges

**📍 [docs/operations/CLUSTER-BOOTSTRAP.md](../docs/operations/CLUSTER-BOOTSTRAP.md)**

## Bootstrap Process Overview

This operational guide provides step-by-step instructions for bootstrapping an OpenShift cluster with Argo CD.
...
ACM documentation has been consolidated to:2. Configuring Argo CD with cluster-admin permissions

3. Creating external secrets (if using External Secrets Operator with Infisical)

- **📍 [docs/operations/acm/](../docs/operations/acm/)** - All ACM operational guides4. Creating the initial "cluster" Application pointing to `roles/<cluster-name>/`

- **📍 [scripts/acm/](../scripts/acm/)** - ACM scripts and manifests5. Argo CD automatically deploying ApplicationSets from the role chart

- **📍 [docs/ACM-GETTING-STARTED.md](../docs/ACM-GETTING-STARTED.md)** - ACM introduction6. ApplicationSets creating individual Applications for each enabled app

### Quick Links## Architecture Overview

- **🚀 Quick Start:** [GETTING-STARTED.md](../GETTING-STARTED.md)This repository uses the **Red Hat Validated Patterns Framework** with a three-level GitOps architecture:

- **📚 Complete Documentation:** [docs/INDEX.md](../docs/INDEX.md)

- **🏗️ Architecture Overview:** [docs/DETAILED-OVERVIEW.md](../docs/DETAILED-OVERVIEW.md)- **Bootstrap** (manual): Create one Argo CD `Application` named "cluster" pointing to `roles/<cluster-name>/` with values from `values-<cluster>.yaml`

- **Roles** (Helm charts): Each cluster role deploys **Applications that deploy ApplicationSets** (platform, ai, media, etc.)

---- **ApplicationSets** (Helm charts): Master ApplicationSets in `charts/platform/` and `charts/applications/<domain>/` generate child Applications

- **Applications**: Individual application Helm charts in `charts/applications/<domain>/<app>/`

**Why the change?**

**Configuration hierarchy:**

The reorganization follows the documented Strategic → Tactical → Operational pattern:

- `values-global.yaml` - Pattern-wide defaults

- **Strategic (WHY):** ADR 002 explains why Validated Patterns Framework- `values-<cluster>.yaml` - Cluster-specific overrides (sno, hub, test)

- **Tactical (WHAT):** docs/DETAILED-OVERVIEW.md explains what the architecture is

- **Operational (HOW):** docs/operations/ contains step-by-step instructions**Available cluster roles:**

This eliminates duplication and provides a single source of truth for each documentation type.- `sno` - Single Node OpenShift configuration
...
Create the main cluster Application pointing to your chosen role using the **Validated Patterns values file approach**. The `path` should be `roles/<cluster-role>` where cluster-role is one of: `sno`, `hub`, `test`, or `template`.
...
> **Important:**
>
> - Change `path: roles/sno` to match your cluster role (`roles/hub`, `roles/test`, etc.)
> - Change `valueFiles` to match: `../../clusters/individual-clusters/values-hub.yaml`, `../../clusters/individual-clusters/values-test.yaml`, etc.
> - Update `repoURL` to point to your GitHub repository

## Step 5: Customize Configuration

Configuration is now managed in `values-<cluster>.yaml` files using the **Validated Patterns structure**.

**Before deploying, customize your `values-<cluster>.yaml` file:**

1. **Update Git repository URL** in `spec.source.repoURL`:
...
2. **Configure cluster settings** in `clusterGroup`:

   ```yaml
   clusterGroup:
     name: sno # or hub, test
     cluster:
       top_level_domain: example.com # Your domain
       admin_email: admin@example.com
       timezone: America/New_York
   ```

3. **Enable/disable platform components** in `clusterGroup.platformComponents`:
...
4. **Enable/disable applications** in `clusterGroup.applicationStacks`:
   ```yaml
   applicationStacks:
     ai:
       enabled: true
...
See `values-prod.yaml`, `values-hub.yaml`, and `values-test.yaml` for complete examples.

## Step 6: Deploy and Verify

After applying the cluster Application, Argo CD will follow the **Validated Patterns deployment flow**:

1. **Deploy role chart** from `roles/<cluster-name>/` using values from `values-<cluster>.yaml`
2. **Create ApplicationSet deployers** (Applications that deploy ApplicationSets):
   - `platform-applicationset` (sync-wave: 10) - Deploys platform ApplicationSet chart
   - `ai-applicationset`, `media-applicationset`, etc. (sync-wave: 50) - Deploy application domain ApplicationSets
3. **ApplicationSets generate Applications** for enabled components:
   - **Platform** (sync-wave: 0-200): ESO, certificates, storage, VPA, Goldilocks, Gatus, GPU operators, etc.
   - **Applications** (sync-wave: 100): AI, Media, Home Automation, Productivity apps
4. **Applications deploy to namespaces** - Each app deploys resources from `charts/applications/<domain>/<app>/`
...
2. **App-specific config**: Edit `charts/applications/<domain>/<app>/values.yaml` for app defaults
...
4. **Add custom applications**: Follow the guide in `.github/instructions/adding-application-checklist.md`
...
- **Adding Applications**: `.github/instructions/adding-application-checklist.md`
