...
cleanup_kasten() {
    ...
    if oc get k10.apik10.kasten.io/k10 -n kasten-io &>/dev/null; then
        ...
        oc delete k10.apik10.kasten.io/k10 -n kasten-io 2>/dev/null || true
    fi

    # Remove Kasten subscription
    local kasten_sub=$(oc get subscriptions -n kasten-io 2>/dev/null | grep "kasten" | awk '{print $1}')
    if [ -n "$kasten_sub" ]; then
        log_info "  Removing Kasten subscription..."
        echo "$kasten_sub" | xargs oc delete subscription -n kasten-io 2>/dev/null || true
        log_success "  Kasten subscription removed"
    fi

    # Remove Kasten CSVs from all namespaces
    log_info "  Removing Kasten ClusterServiceVersions..."
    local kasten_csvs=$(oc get csv --all-namespaces -o custom-columns=NS:.metadata.namespace,NAME:.metadata.name --no-headers 2>/dev/null | \
                        grep "kasten" || true)
    if [ -n "$kasten_csvs" ]; then
        while IFS= read -r line; do
            ns=$(echo $line | awk '{print $1}')
            name=$(echo $line | awk '{print $2}')
            log_info "    Deleting CSV $name from namespace $ns..."
            oc delete csv $name -n $ns 2>/dev/null || true
        done <<< "$kasten_csvs"
        log_success "  Kasten CSVs removed"
    fi

    # Remove Kasten Operator CR
    local kasten_operator=$(oc get operators 2>/dev/null | grep "kasten" | awk '{print $1}')
    if [ -n "$kasten_operator" ]; then
        log_info "  Removing Kasten Operator CR..."
        echo "$kasten_operator" | xargs oc delete operator 2>/dev/null || true
        log_success "  Kasten Operator CR removed"
    fi
    ...
    # Remove all install plans (including completed ones to prevent operator recreation)
    local install_plans=$(oc get installplan -n kasten-io 2>/dev/null | grep "kasten" | awk '{print $1}')
    if [ -n "$install_plans" ]; then
        log_info "  Removing install plans..."
        echo "$install_plans" | xargs oc delete installplan -n kasten-io 2>/dev/null || true
        log_success "  Kasten install plans removed"
    fi
...
cleanup_external_secrets() {
    ...
    # Remove ESO CSVs from all namespaces
    log_info "  Removing ESO ClusterServiceVersions..."
    local eso_csvs=$(oc get csv --all-namespaces -o custom-columns=NS:.metadata.namespace,NAME:.metadata.name --no-headers 2>/dev/null | \
                     grep "external-secrets-operator" || true)
    if [ -n "$eso_csvs" ]; then
        while IFS= read -r line; do
            ns=$(echo $line | awk '{print $1}')
            name=$(echo $line | awk '{print $2}')
        ...
        log_success "  ESO CSVs removed"
    fi

    # Remove ESO Operator CR
    local eso_operator=$(oc get operators 2>/dev/null | grep "external-secrets-operator" | awk '{print $1}')
    if [ -n "$eso_operator" ]; then
        log_info "  Removing ESO Operator CR..."
        echo "$eso_operator" | xargs oc delete operator 2>/dev/null || true
        log_success "  ESO Operator CR removed"
    fi
    ...
    # Remove all install plans (including completed ones to prevent operator recreation)
    local install_plans=$(oc get installplan -n openshift-operators 2>/dev/null | grep "external-secrets" | awk '{print $1}')
    if [ -n "$install_plans" ]; then
        log_info "  Removing install plans..."
        echo "$install_plans" | xargs oc delete installplan -n openshift-operators 2>/dev/null || true
        log_success "  ESO install plans removed"
    fi
...
cleanup_cert_manager() {
    ...
    # Remove cert-manager CSVs from all namespaces
    log_info "  Removing cert-manager ClusterServiceVersions..."
    local cm_csvs=$(oc get csv --all-namespaces -o custom-columns=NS:.metadata.namespace,NAME:.metadata.name --no-headers 2>/dev/null | \
    ...
    if [ -n "$cm_csvs" ]; then
        ...
        log_success "  cert-manager CSVs removed"
    fi

    # Remove cert-manager Operator CR
    local cm_operator=$(oc get operators 2>/dev/null | grep "cert-manager" | awk '{print $1}')
    if [ -n "$cm_operator" ]; then
        log_info "  Removing cert-manager Operator CR..."
        echo "$cm_operator" | xargs oc delete operator 2>/dev/null || true
        log_success "  cert-manager Operator CR removed"
    fi

    # Remove cert-manager CRDs
    local cm_crds=$(oc get crd 2>/dev/null | grep "cert-manager.io" | awk '{print $1}')
    if [ -n "$cm_crds" ]; then
        log_info "  Removing cert-manager CRDs..."
        echo "$cm_crds" | xargs oc delete crd 2>/dev/null || true
        log_success "  cert-manager CRDs removed"
    fi
    ...
    # Remove all install plans (including completed ones to prevent operator recreation)
    local install_plans=$(oc get installplan -n openshift-operators 2>/dev/null | grep "cert-manager" | awk '{print $1}')
    if [ -n "$install_plans" ]; then
        log_info "  Removing install plans..."
        echo "$install_plans" | xargs oc delete installplan -n openshift-operators 2>/dev/null || true
        log_success "  cert-manager install plans removed"
    fi
