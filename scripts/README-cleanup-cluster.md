...
3. **Kasten K10 Cleanup**
   ...
   - Kasten subscriptions preventing clean uninstall
   - Stale CSVs (ClusterServiceVersions) across all namespaces
...
6. **cert-manager Resources**
   ...
   - Stale CSVs (ClusterServiceVersions) across all namespaces
   - Stale CRDs preventing clean uninstall
...
- Removes finalizers from k10 custom resource and deletes it
- Removes Kasten subscription
- Removes all Kasten ClusterServiceVersions (CSVs) from all namespaces
- Removes Kasten Operator CR (OLM tracking resource)
- Removes Kasten API services (e.g., `v1alpha1.*.kio.kasten.io`)
- Deletes all Kasten CRDs
- Deletes all install plans (prevents operator recreation)
- Allows kasten-io namespace to terminate

**Note:** The Operator CR may be recreated by OLM for tracking purposes, but this doesn't represent actual running workloads. Check `oc get deployments,pods -A | grep kasten` to verify the operator is truly removed.
...
- Removes all ESO ClusterServiceVersions (CSVs) from all namespaces
- Removes ESO Operator CR (OLM tracking resource)
- Removes ESO CRDs
- Cleans up ESO API services
- Deletes all install plans (prevents operator recreation)
- Resolves conversion webhook issues during upgrades

**Note:** The Operator CR may be recreated by OLM for tracking purposes, but this doesn't represent actual running workloads. Check `oc get deployments,pods -A | grep external-secrets` to verify the operator is truly removed.
...
- Removes all cert-manager ClusterServiceVersions (CSVs) from all namespaces
- Removes cert-manager Operator CR (OLM tracking resource)
- Removes all cert-manager CRDs
- Removes webhook configurations that may block deletion
- Deletes all install plans (prevents operator recreation)

**Note:** The Operator CR may be recreated by OLM for tracking purposes, but this doesn't represent actual running workloads. Check `oc get deployments,pods -A | grep cert-manager` to verify the operator is truly removed.
