...
**Step-by-step instructions for bootstrapping an OpenShift cluster with Argo CD using this GitOps repository.**

> **📋 Architecture:** See [DETAILED-OVERVIEW.md](../DETAILED-OVERVIEW.md) for complete architecture explanation
> **📋 Strategic Context:** See [ADR 002: Validated Patterns Framework](../decisions/002-validated-patterns-framework.md) for architectural rationale
> **⚡ Quick Start:** See [GETTING-STARTED.md](../../GETTING-STARTED.md) for a faster 30-minute setup
...
This operational guide walks through the manual bootstrap process:

1. Install the OpenShift GitOps operator (Argo CD) on the cluster
2. Configure Argo CD with cluster-admin permissions
3. Create external secrets (if using External Secrets Operator with Infisical)
4. Create the initial "cluster" Application pointing to `roles/<cluster-name>/`
5. Argo CD automatically deploys ApplicationSets from the role chart
6. ApplicationSets create individual Applications for each enabled app
